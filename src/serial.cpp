...
SerialMode_ serialGetConfig(uint8_t dataBits, ParityType parity, uint8_t stopBits, bool flowControl) {
    SerialMode_ config = M_SERIAL_8N1;
    ...
    // Parity
    if (parity == ODD) {
        config = static_cast<SerialMode_>(config | 0x1);
    } else if (parity == EVEN) {
        config = static_cast<SerialMode_>(config | 0x2);
    }
